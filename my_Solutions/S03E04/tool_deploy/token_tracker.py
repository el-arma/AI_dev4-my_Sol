...
class CostTracker:
    ...
    def sum_tokens(self, model: Model | str | None, usage: RunUsage) -> None:
        if model is None:
            return
        model_str = model if isinstance(model, str) else model.model_name
        key = _normalize(model_str)
    ...
    def as_dict(self) -> dict:
        ...
        for model, s in self._stats.items():
            ...
            cost = (s.input_tokens * price[0] + s.output_tokens * price[1]) / 1_000_000 if price else None
            result[model] = {
