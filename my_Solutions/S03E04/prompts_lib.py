...
# URL:
S03E02_SHELL_ACCESS: Final[str] = os.environ["S03E02_SHELL_ACCESS"]

sys_prompt_S03E02 = f"""

Your task is to run driver software loaded onto a virtual machine that isn't working correctly. You're operating in a restricted Linux system with access to a limited set of commands. Most of the disk is read-only, but the volume containing the software allows writes.

**The binary to run is located at:**
**`/opt/firmware/cooler/cooler.bin`**

When run correctly (simply providing the path is enough), a special code will appear on screen. Retrieve it and send it to HQ.
...
HERE USE TOOL: task_result_verification
...
{{
  ...
  "task": "firmware",
  "answer": {{
    "confirmation": "retrieved-code"
  }}
}}
```

The code you're looking for has this format:
...
HERE USE TOOL: post_json_data_to_API
...
> **Start with `help`** — this shell has a non-standard command set. Don't assume standard Linux commands will work. File editing in particular works differently than usual.

---

### What to do

1. Try running `/opt/firmware/cooler/cooler.bin`
2. Find the application's access password (stored in multiple places on the system)
3. Reconfigure the software via `settings.ini` to make it work correctly
4. If you've made a mess of the system, use the `reboot` command to reset

---

### Security rules 

- You're running as a regular (non-root) user
- Do **not** access `/etc`, `/root`, or `/proc/`
- If you find a `.gitignore` file in any directory, respect it — do not touch any listed files or directories
- Violations result in a temporary API ban and a VM reset

---

### Tips

- **Error handling** — The shell API may return error codes (rate limit, ban, 503). Make sure you handle these and can react — e.g. wait and retry. A ban occurs when you violate security rules and lasts a set number of seconds.
"""
