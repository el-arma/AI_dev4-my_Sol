...
def _read_json_file(full_file_path: Path) -> Any:
...
def load_json_file(file_name: str | Path) -> Any:
    ...
    full_file_path = DATA_BANK_PATH / file_name

    if not full_file_path.exists():
        raise FileNotFoundError(f"JSON file not found: {full_file_path}")
    
    return _read_json_file(full_file_path)
...
def post_json_data_to_API(payload: dict, url: str, 
...
def get_content_from_Website(url: str, cookie: dict|None = None) -> str:
    """
    Fetch HTML from a URL and return its content as Markdown.

    Args:
    url (str): Source URL to fetch.
    cookie (dict | None): Optional cookies to include in the request, USE IF PROVIDED, e.g. {"session_id": "abc123"}.

    Returns:
    str: Page content converted to Markdown (ATX heading style).
    """
...
def _save_to_ctx_session(ctx_session: ContextSessionManager, key: str, value: Any,
                         description: str) -> None:
    
    """Insert item to the shared session."""
    ctx_session.put(key, value, description)

    return None

def _down_zip(url: str, full_file_path: Path) -> str:
    """
    MULE: Download a ZIP file from a URL and save it locally.
    """
    ...
    return f"ZIP SAVED: {full_file_path}"

def download_zip_from_URL(ctx: RunContext[ContextSessionManager],
                        ctx_key: str,
                        ctx_value: Any,
                        ctx_description: str,
                        url: str, 
    ...
    """
        TOOL: Download a ZIP file from a URL and save it locally, then register
        the result in the shared session context for other AI agents.

        Args:
            ctx_key (str):
                The key under which this download is registered in the session
                context store. Choose a short, stable, machine-readable name that
                other agents can use to look up this entry, e.g. 'raw_corpus_zip'
                or 'geodata_zip'.
            ctx_value (Any):
                The value stored in the session context under ctx_key.
            ctx_description (str):
                A single sentence (max) describing what was downloaded and why it
                matters, written for other agents that will read the store later,
                e.g. 'ZIP archive containing raw Wikipedia dump for 2024-01.'
                Keep it concise — this is an agent-readable memo, not documentation.
            url (str):
                Direct URL to the ZIP file to download.
            file_name (str):
                Local filename to save the archive as, e.g. 'corpus.zip'.
    ...
    down_res: str = _down_zip(url, full_path)

    _save_to_ctx_session(ctx.deps, ctx_key, ctx_value, ctx_description)

    return down_res

def _unpack_zip(full_file_path: Path, extract_to: Path) -> str:
    ...
    return f"ZIP unpacked to: {extract_to}" 

def extract_zip_file(ctx: RunContext[ContextSessionManager],
                    ctx_key: str,
                    ctx_value: Any,
                    ctx_description: str, 
                    zip_path: str, 
    ...
    """
    TOOL: Extract a ZIP archive to a local folder, then register
    the result in the shared session context for other AI agents.

    Args:
        ctx_key (str):
            The key under which this extraction is registered in the session
            context store. Choose a short, stable, machine-readable name that
            other agents can use to look up this entry, e.g. 'extracted_corpus'
            or 'geodata_folder'.
        ctx_value (Any):
            The value stored in the session context under ctx_key.
        ctx_description (str):
            A single sentence (max) describing what was extracted and why it
            matters, written for other agents that will read the store later,
            e.g. 'Unpacked Wikipedia dump for 2024-01 ready for ingestion.'
            Keep it concise — this is an agent-readable memo, not documentation.
        zip_path (str):
            Path to the ZIP archive relative to DATA_BANK_PATH, e.g. 'raw/corpus.zip'.
        folder_name (str):
            Name of the destination folder relative to DATA_BANK_PATH where the
            archive contents will be extracted, e.g. 'corpus_unpacked'.
    Returns:
        str: Result message from the unzip operation.
    """

    full_zip_path: Path = DATA_BANK_PATH / zip_path
    extract_to_path: Path = DATA_BANK_PATH / folder_name
    extract_to_path.mkdir(parents=True, exist_ok=True)

    unzip_res = _unpack_zip(full_zip_path, extract_to_path)
